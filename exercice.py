...
def build_print_note_name_callback(midi_to_name):
	...
	def callback(midi_msg):
		# si j'ai une note et une velocite >0, alors j'affiche le nom de la note associe au numero (grace au ditc midi_to_name)
...
def build_print_chord_name_callback(chord_names_and_notes, name_to_midi):
	...
	for name, notes in chord_names_and_notes.items():
		chords_notes = [False] * 12
		for note in notes:
			chords_notes[name_to_midi[note]%NOTES_PER_OCTAVE] = True
		chords[tuple(chords_notes)] = name
	# Créez et retourner le callback
	def callback(midi_msg):
		global note_state
		if midi_msg.type == 'note_on' and midi_msg.velocity > 0:
			note_state[midi_msg.note % NOTES_PER_OCTAVE]= True
			notes_state_tuple = tuple(note_state)
			if notes_state_tuple in chords:
				print(chords[notes_state_tuple])
		elif midi_msg.type == 'note_off' or (midi_msg.type == 'note_on' and midi_msg.velocity == 0):
			note_state[midi_msg.note % NOTES_PER_OCTAVE] = False
